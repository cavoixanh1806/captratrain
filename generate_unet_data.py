...
def generate_warm_gray_background(size: int = 128) -> np.ndarray:
    """Sinh nen giong real CAPTCHA — 45% gray, 30% warm, 14% green, 5% blue.

    Real: BG sat median=44, val avg=180, per-channel std=6.6.
    Key: nen co MÀU NHẸ, khong chi pure gray.
    ...
    base_v = random.randint(155, 210)  # val avg=180

    if bg_roll < 0.45:
        # Gray (45%): sat 5-35
        bg_h = random.randint(0, 179)
        bg_s = random.randint(8, 35)
    elif bg_roll < 0.75:
        # Warm/sand (30%): hue 10-30, sat 35-80
        bg_h = random.randint(10, 30)
        bg_s = random.randint(35, 80)
    elif bg_roll < 0.89:
        # Green-tinted (14%): hue 35-80, sat 30-70
        bg_h = random.randint(35, 80)
        bg_s = random.randint(30, 70)
    elif bg_roll < 0.94:
        # Blue-tinted (5%): hue 90-130, sat 35-90
        bg_h = random.randint(90, 130)
        bg_s = random.randint(35, 90)
    else:
        # Other (6%): random hue
        bg_h = random.randint(0, 179)
        bg_s = random.randint(25, 65)

    # Convert HSV -> BGR base color
    hsv_base = np.uint8([[[bg_h, bg_s, base_v]]])
    bgr_base = cv2.cvtColor(hsv_base, cv2.COLOR_HSV2BGR)[0, 0].astype(np.float32)
    img = np.full((size, size, 3), bgr_base, dtype=np.float32)

    # === Texture: per-channel variation (real std=6.6) ===
    texture_roll = random.random()

    if texture_roll < 0.55:
        noise = np.random.normal(0, random.uniform(4, 8), img.shape).astype(np.float32)
        img += noise
    elif texture_roll < 0.90:
        # Gradient + noise (mild texture)
        gradient_strength = random.uniform(8, 20)
        ...
        noise = np.random.normal(0, random.uniform(3, 6), img.shape).astype(np.float32)
        img += noise
    else:
        # Patches (map-like)
        ...
        for _ in range(num_patches):
            ...
            # Patch color variation — bigger than before
            patch_shift = np.array([
                random.randint(-25, 25),
                random.randint(-25, 25),
                random.randint(-25, 25),
            ], dtype=np.float32)
            patch_color = bgr_base + patch_shift
            y1, y2 = max(0, cy - patch_size), min(size, cy + patch_size)
            x1, x2 = max(0, cx - patch_size), min(size, cx + patch_size)
            if y2 > y1 and x2 > x1:
                yy, xx = np.ogrid[y1:y2, x1:x2]
                dist = np.sqrt((xx - cx) ** 2 + (yy - cy) ** 2)
                alpha = np.clip(1 - dist / patch_size, 0, 1) * 0.35
                for c in range(3):
                    img[y1:y2, x1:x2, c] = (
                        img[y1:y2, x1:x2, c] * (1 - alpha)
                        + patch_color[c] * alpha
                    )
        noise = np.random.normal(0, random.uniform(3, 7), img.shape).astype(np.float32)
        img += noise
